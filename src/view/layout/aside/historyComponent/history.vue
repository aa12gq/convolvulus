...
.contextmenu {
  ...
  margin: 0;
  border: none; // 移除边框
  background: #333; // 修改背景颜色为深灰色
  z-index: 3000;
  position: absolute;
  list-style-type: none;
  padding: 5px 0;
  border-radius: 6px; // 圆角大小调整
  font-size: 14px;
  color: #fff; // 文本颜色修改为白色
  box-shadow: 4px 4px 8px 0 rgba(0, 0, 0, 0.3); // 阴影效果增强
}

.el-tabs__item .el-icon-close {
  color: #fff !important; // 关闭按钮颜色改为白色，如果需要的话
}
.el-tabs__item .dot {
  content: "";
  width: 9px;
...
.contextmenu li {
  margin: 0;
  padding: 10px 20px; // 调整内边距，使得菜单项更高一些
}
.contextmenu li:hover {
  background: #555; // 鼠标悬停时的背景颜色变深
  cursor: pointer;
}
...
.gva-tab {
  .el-tabs__item.is-active {
    color: #409EFF !important; // 设置激活的标签文字颜色
    border-bottom-color: #409EFF !important; // 激活状态下的底部边框颜色
  }

  .el-tabs__item {
    padding: 10px 15px; // 设置标签项的内边距
    &:hover {
